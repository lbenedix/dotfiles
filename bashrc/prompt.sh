...
__ps1_rvm() {
	if [[ -n ${rvm_path} ]]; then
		echo -e " ${PCOL_yellow}rvm:$(${rvm_path}/bin/rvm-prompt i v p g s)${PCOL_none}"
	fi
}
...
PS1="${COL_lcyan}\t${COL_none}"
PS1="${PS1} ${COL_lgray}[${COL_none}\$(__ps1_rc \$?)${COL_lgray}]${COL_none}"
PS1="${PS1} ${COL_user}\u${COL_yellow}@${COL_lgreen}${_NODENAME}${COL_lgray}.${_DOMAINNAME}${COL_none}"
